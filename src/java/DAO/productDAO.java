...
public class productDAO extends DBContext {
    ...
    public static void main(String[] args) {
       productDAO c = new productDAO();
        ...
        List<Product> list = c.getProductbyCategory(1);
        System.out.println(list.get(0).getCategory().getCategory_name());

    }

    public List<Sex> getBuySex() {
        List<Sex> list = new ArrayList<>();
        String sql = "SELECT [product_id]\n"
                + "      ,[sex]\n"
                + "  FROM [dbo].[product_sex]\n"
                + "  where product_sex.sex = 'Nam'";
        try {
            //conn = new DBContext().getConnection();
            ps = connection.prepareStatement(sql);
            rs = ps.executeQuery();
            while (rs.next()) {
                list.add(new Sex(rs.getString(1), rs.getString(2)));
            }
        } catch (Exception e) {
        }
        return list;
    ...
    public List<Product> getProductbyCategory(int cid) {
        ...
        String sql = "   select  c.category_name , p.product_id , p.product_name, p.product_price, p.product_describe, p.quantity,p.img from product p \n"
                + "inner join category c on p.category_id = c.category_id where 1 = 1";
        if (cid != 0) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                p.setProduct_describe(rs.getString("product_describe"));
    ...
    public void insertProduct(Product product) {
        ...
        String sql2 = "insert into product_sex (product_id,sex) values(?,?)";
        try {
            //conn = new DBContext().getConnection();
            for (Sex i : product.getSex()) {
                ps = connection.prepareStatement(sql2);
                ps.setString(1, i.getProduct_id());
                ps.setString(2, i.getsex());
                ps.executeUpdate();
            }
        } catch (Exception e) {
        }
    ...
    public void ProductDelete(String product_id) {
        ...
        String sql1 = "delete from product_sex where product_id=?";
        try {
            //conn = new DBContext().getConnection();
            ps = connection.prepareStatement(sql1);
            ps.setString(1, product_id);
    ...
    public void updateProduct(Product product) {
        ...
        String sql3 = "delete from product_sex where product_id=?";
        try {
            //conn = new DBContext().getConnection();
        ...
        String sql5 = "insert into product_sex (product_id,sex) values(?,?)";
        try {
            //conn = new DBContext().getConnection();
            for (Sex i : product.getSex()) {
    ...
    public List<Sex> getSex() {
        List<Sex> list = new ArrayList<>();
        String sql = "select * from product_sex";
        try {
            //conn = new DBContext().getConnection();
            ps = connection.prepareStatement(sql);
            rs = ps.executeQuery();
    ...
    public List<Product> getProductAZ() {
        ...
        String sql = "select c.category_name , p.product_id , p.product_name, p.product_price, p.product_describe, p.quantity,p.img"
                + " from product p inner join category c on p.category_id = c.category_id ORDER BY p.product_name";
        try {
            //conn = new DBContext().getConnection();
            ps = connection.prepareStatement(sql);
            rs = ps.executeQuery();
    ...
    public List<Sex> getSexByID(String product_id) {
        List<Sex> list = new ArrayList<>();
        String sql = "select * from product_sex where product_id=?";
        try {
            //conn = new DBContext().getConnection();
            ps = connection.prepareStatement(sql);
            ps.setString(1, product_id);
    ...
    public List<Product> SearchAll(String text) {
        ...
        String sql = "SELECT DISTINCT c.category_name , p.product_id , p.product_name, p.product_price, p.product_describe, p.quantity,p.img \n"
                + "FROM product p inner join category c on c.category_id = p.category_id inner join product_sex ps on p.product_id = ps.product_id\n"
                + "WHERE product_name LIKE ? OR  product_price LIKE ? OR ps.sex LIKE ? OR c.category_name LIKE ?";
